...
class Matrix:
    ...
    def __copy__(self):
        return Matrix(copy.deepcopy(self.__base_matrix))
    ...
    def __mul__(self, other):
        if self.__size_x != other.__size_y:
            return None
        res_matrix = Matrix([[0] * other.__size_x
        ...
        if self.__size_x != other.__size_x or self.__size_y != other.__size_y:
            ...
            for i in range(self.__size_y):
                for j in range(self.__size_x):
                    if not other[i][j] - Matrix.__eps <= self[i][j] \
                           <= other[i][j] + Matrix.__eps:
                        is_eq = False
        return is_eq
    ...
    def lu_decompose(self):
        l = Matrix([[0] * len(self) for _ in range(len(self))])
        u = Matrix([[0] * len(self) for _ in range(len(self))])
    ...
    def inversed(self):
        ...
        """
        n = len(self)
        E = Matrix([[float(i == j) for i in range(n)] for j in range(n)])
        l, u = self.lu_decompose()
        res = []
        for e in E:
            res_row = solve_lu(l, u, e)
            res.append(res_row)
        return Matrix(res).transposed()
    ...
    def solve_system_tridiagonal(self, b):
        ...
        return Matrix(ans).transposed(), to_use
