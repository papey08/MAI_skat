...
from core.mongo_accessor import MongoAccessor
...
class NatsServer:
    def __init__(self, nats_url: str, endpoint: str, access_key: str, secret_key: str, mongo_addr: str):
        self.nc: Optional[nats.NATS] = None
        self.nats_url = nats_url

        self.minio_accessor = MinioAccessor(endpoint, access_key, secret_key)
        self.mongo_accessor = MongoAccessor(mongo_addr)
    ...
    async def handle_create_folder(self, msg):
        ...
        folder = self.mongo_accessor.create_folder(create_folder.user_id, create_folder.path)
    ...
    async def handle_create_file(self, msg):
        ...
        file, file_id = self.mongo_accessor.create_file(create_file.user_id, create_file.path, create_file.content)
        self.minio_accessor.create_file(create_file.user_id, file_id, create_file.content)
    ...
    async def handle_delete_file(self, msg):
        ...
        _, file_id = self.mongo_accessor.get_file(delete_file.user_id, delete_file.path)
        self.mongo_accessor.delete_file(delete_file.user_id, delete_file.path)
        self.minio_accessor.delete_file(delete_file.user_id, file_id)
    ...
    async def handle_delete_folder(self, msg):
        ...
        folder = self.mongo_accessor.get_folder(delete_folder.user_id, delete_folder.path)
        for file in folder.files:
            _, file_id = self.mongo_accessor.get_file(delete_folder.user_id, file.path)
            self.minio_accessor.delete_file(delete_folder.user_id, file_id)
        self.mongo_accessor.delete_folder(delete_folder.user_id, delete_folder.path)
    ...
    async def handle_get_file(self, msg):
        ...
        file, file_id = self.mongo_accessor.get_file(get_file.user_id, get_file.path)
        content = self.minio_accessor.get_file(get_file.user_id, file_id)
    ...
    async def handle_get_files(self, msg):
        ...
        files = self.mongo_accessor.get_files(get_files.user_id, get_files.file_name, get_files.limit, get_files.offset)
    ...
    async def handle_get_folder(self, msg):
        ...
        folder = self.mongo_accessor.get_folder(get_folder.user_id, get_folder.path)
