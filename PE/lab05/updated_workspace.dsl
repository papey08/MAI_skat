workspace {
    ...
    model {
        ...
        file_storage_service = softwareSystem "file_storage_service" {
            ...
            core_service = container "core_service" {
                ...
                core_mongo_accessor = component "mongo_accessor" {
                    description "Класс для работы с MongoDB"
                    technology "MongoDB"
                }

                core_nats_server -> core_user_accessor
                core_nats_server -> core_mongo_accessor
            ...
            user_service = container "user_service" {
                ...
                user_accessor = component "user_accessor" {
                    description "Класс для получения/создания пользователей"
                }

                user_redis_accessor = component "cache_accessor" {
                ...
                user_nats_server -> user_accessor
                user_accessor -> user_redis_accessor
                user_accessor -> user_postgres_accessor
            }
            ...
            user_redis = container "user_redis" {
                description "Кэш-хранилище пользователей"
                technology "redis"
                tags "db, user"
            }

            core_mongodb = container "core_mongodb" {
                description "База данных MongoDB для хранения файловой структуры пользователя"
                technology "mongodb"
                tags "db, core"
            }
            ...
            user_redis_accessor -> user_redis

            auth_service -> auth_redis
            auth_redis_accessor -> auth_redis

            core_service -> core_mongodb
            core_mongo_accessor -> core_mongodb
    ...
    views {
        ...
        dynamic file_storage_service "getting_file_usecase" {
            ...
            core_service -> core_mongodb "Выполняется поиск в БД по названию файла и id владельца"
            core_mongodb -> core_service "БД возвращает список id из найденных файлов, соответствующих запросу"
